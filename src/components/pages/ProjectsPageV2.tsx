    ...
    <main className="min-h-screen bg-[#f8f7f2] text-[#17232a]">
      ...
      <section id="catalog-results" className="scroll-mt-20 px-5 py-10 sm:px-8 lg:px-10 lg:py-14">
        <div className="mx-auto max-w-[1440px]">
          ...
          {hasFilters && <div className="mt-4 flex items-center gap-2"><span className="text-xs text-[#777a72]">Recherche active</span><button type="button" onClick={clearFilters} className="inline-flex min-h-11 items-center gap-1 text-xs font-semibold text-[#183c31]"><X className="size-3" />Tout effacer</button></div>}
          ...
          {!isLoading && filteredProjects.length === 0 && <div className="mt-8 rounded-2xl border border-[#e5e1d7] bg-white px-6 py-14 text-center"><Building2 className="mx-auto size-8 text-[#6c806f]" /><h3 className="mt-4 text-xl font-semibold">Aucun projet ne correspond à ces critères.</h3><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-[#777a72]">Élargissez votre recherche ou échangez directement avec un conseiller ASAS pour identifier les options pertinentes.</p><Button onClick={clearFilters} className="mt-6 min-h-11 bg-[#17232a] text-white">Réinitialiser</Button></div>}
        </div>
      </section>
