    ...
    <main className="min-h-screen bg-ivory text-charcoal">
        ...
        <div className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:px-12 lg:py-24">
          ...
          <div className="mt-5 grid gap-10 lg:grid-cols-[1fr_.8fr] lg:items-end">
            <div>
              ...
              <p className="mt-6 max-w-2xl text-base leading-7 text-white/70 sm:text-lg">Que vous cherchiez un logement ou souhaitiez parler d’une opération immobilière, choisissez le moyen de contact qui vous convient.</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
              <p className="text-sm font-medium text-white/60">ASAS</p>
              <p className="mt-2 text-xl font-medium">{ASAS.fullName}</p>
              <p className="mt-2 text-sm text-white/60">{ASAS.tagline}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="border-b border-border bg-white">
        <div className="mx-auto grid max-w-7xl gap-px px-5 sm:px-8 md:grid-cols-3 lg:px-12">
          ...
          <a href={getPhoneUrl()} className="group flex items-center gap-4 border-t border-border py-7 md:border-l md:border-t-0 md:px-7">
          ...
          <a href={`mailto:${ASAS.email}`} className="group flex items-center gap-4 border-t border-border py-7 md:border-l md:border-t-0 md:px-7">
      ...
      <section className="mx-auto grid max-w-7xl gap-14 px-5 py-20 sm:px-8 lg:grid-cols-[.72fr_1.28fr] lg:px-12 lg:py-28">
        <div>
          ...
          <p className="mt-5 text-base leading-7 text-muted-foreground">Le formulaire permet de transmettre votre demande sans multiplier les étapes. Ajoutez les informations utiles à votre recherche ou à votre projet.</p>
          <div className="mt-9 space-y-5">
            ...
            <div className="rounded-xl border border-border bg-sand/25 p-5"><p className="text-sm font-semibold">Après votre message</p><p className="mt-2 text-sm leading-6 text-muted-foreground">Votre demande est transmise à l’équipe ASAS afin de poursuivre l’échange avec le contexte que vous avez fourni.</p></div>
          </div>
        </div>
