...
export default function ApartmentDetailPageV3({ projectSlug, apartmentSlug }: Props) {
  ...
  const isAvailable = apartment.status === 'AVAILABLE' || apartment.status === 'COMING_SOON';
  const whatsappMessage = `Bonjour, je suis intéressé(e) par l'appartement ${apartment.typeName} (${formatSurface(apartment.surface)}) dans le projet ${project?.name ?? ''}. Je souhaite demander une visite.`;
      ...
      <div className="mx-auto max-w-[1440px] px-5 pt-5 sm:px-8 lg:px-10">
        ...
        <div className="grid gap-8 py-7 lg:grid-cols-[0.9fr_1.1fr] lg:items-center lg:py-10">
          <div className="order-2 lg:order-1">
            ...
            <div className="mt-7 flex flex-col gap-2.5 sm:flex-row"><a href={isAvailable ? getWhatsAppUrl(whatsappMessage) : getWhatsAppUrl(`Bonjour, je souhaite connaître les alternatives disponibles dans le projet ${project?.name ?? ''}.`)} target="_blank" rel="noopener noreferrer" onClick={() => trackEvent('apartment_cta_click', { apartment_id: apartment.id, cta: 'whatsapp_hero' })} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-md bg-[#d9b16e] px-6 text-sm font-semibold text-[#17232a] hover:bg-[#e4bd7c]"><MessageCircle className="size-4" />{isAvailable ? 'Demander une visite' : 'Voir les alternatives'}</a><a href={getPhoneUrl()} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-md border border-white/20 px-5 text-sm font-semibold text-white hover:bg-white/10"><Phone className="size-4" />Appeler</a></div>
