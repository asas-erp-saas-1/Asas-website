...
import { Search, X, Sparkles, ArrowUpDown, MapPin, Building2, Home, Filter, Trash2, Ruler, ArrowUpNarrowWide, ArrowDownWideNarrow, Calendar, Map, Maximize2, FolderSearch, LayoutGrid, SlidersHorizontal, ChevronDown } from 'lucide-react';
...
export default function PublicProjectsCatalogPage() {
  ...
  return (
    <main className="min-h-screen bg-background">
      <section className="border-b border-border bg-ivory px-4 py-8 md:py-10">
        <div className="mx-auto max-w-6xl">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-2xl">
              <p className="mb-2 text-xs font-semibold uppercase tracking-[0.18em] text-forest">Sélection ASAS</p>
              <h1 className="text-3xl font-bold tracking-[-0.025em] text-charcoal md:text-4xl">Nos projets immobiliers</h1>
              <p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground md:text-base">Découvrez les programmes immobiliers commercialisés par ASAS et trouvez un bien correspondant à votre projet.</p>
            </div>
            <div className="flex w-full items-center gap-2 lg:w-auto">
              <div className="relative min-w-0 flex-1 lg:w-[22rem]">
                <Search className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                <Input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Projet, quartier, ville..." className="h-11 bg-white pl-10 pr-10 shadow-sm" aria-label="Rechercher un projet" />
                {searchQuery && <button type="button" onClick={() => setSearchQuery('')} className="absolute right-2 top-1/2 flex size-9 -translate-y-1/2 items-center justify-center rounded-md text-muted-foreground hover:bg-muted hover:text-foreground" aria-label="Effacer la recherche"><X className="h-3.5 w-3.5" /></button>}
              </div>
              <Button variant="outline" size="icon" className="size-11 shrink-0 bg-white" onClick={() => setShowAiSearch((v) => !v)} aria-label="Recherche par IA" aria-expanded={showAiSearch}><Sparkles className="h-4 w-4" /></Button>
            </div>
          </div>
        </div>
      </section>
      <AnimatePresence>{showAiSearch && <motion.section initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden border-b border-border bg-sand/20"><div className="mx-auto max-w-4xl px-4 py-7 text-center"><h2 className="mb-1 flex items-center justify-center gap-2 text-lg font-semibold"><Sparkles className="h-4 w-4 text-gold" />Recherche intelligente</h2><p className="mb-4 text-sm text-muted-foreground">Décrivez ce que vous cherchez en langage naturel</p><AISearch /></div></motion.section>}</AnimatePresence>
      <RecentlyViewedSection />
      <section className="border-b border-border bg-background px-4 py-6"><div className="mx-auto max-w-6xl"><div className="mb-3 flex items-center justify-between"><div className="flex items-center gap-2"><Map className="h-5 w-5 text-forest" /><h2 className="text-lg font-bold">Carte des projets</h2></div><Button variant="outline" size="sm" onClick={() => document.querySelector('[data-map-container]')?.requestFullscreen?.()} className="min-h-10 gap-1.5"><Maximize2 className="h-3.5 w-3.5" /><span className="hidden sm:inline">Plein écran</span></Button></div>{isLoading ? <div className="h-64 animate-pulse rounded-xl border bg-muted md:h-96" /> : <ProjectMap projects={(projects ?? []).filter((p) => p.latitude != null && p.longitude != null).map((p) => ({ slug: p.slug, name: p.name, status: p.status, district: p.district, city: p.city, latitude: p.latitude!, longitude: p.longitude! }))} />}</div></section>
      <section className="border-b border-border bg-background px-4 py-3"><div className="mx-auto max-w-6xl"><div className="flex flex-wrap items-center gap-2"><div className="flex min-w-0 flex-1 flex-wrap gap-1.5"><Button variant={selectedDistrict === '' ? 'default' : 'outline'} size="sm" onClick={() => setSelectedDistrict('')} className={selectedDistrict === '' ? 'min-h-10 bg-forest text-xs text-white' : 'min-h-10 text-xs'}>Tous</Button>{districts.map((d) => <Button key={d} variant={selectedDistrict === d ? 'default' : 'outline'} size="sm" onClick={() => setSelectedDistrict(selectedDistrict === d ? '' : d)} className={selectedDistrict === d ? 'min-h-10 bg-forest text-xs text-white' : 'min-h-10 text-xs'}><MapPin className="mr-1 h-3 w-3" />{d}</Button>)}</div><Button variant="outline" size="sm" className="min-h-10 gap-1.5 text-xs" onClick={() => setShowFilters((v) => !v)} aria-expanded={showFilters}><SlidersHorizontal className="h-3.5 w-3.5" />Filtres<ChevronDown className={`h-3 w-3 transition-transform ${showFilters ? 'rotate-180' : ''}`} /></Button><Select value={sortBy} onValueChange={(v) => setSortBy(v as SortOption)}><SelectTrigger className="min-h-10 w-[160px] text-xs"><ArrowUpDown className="mr-1 h-3 w-3" /><SelectValue /></SelectTrigger><SelectContent>{Object.entries(SORT_OPTIONS).map(([key, { label, icon: Icon }]) => <SelectItem key={key} value={key}><span className="flex items-center gap-2"><Icon className="h-3.5 w-3.5" />{label}</span></SelectItem>)}</SelectContent></Select></div>{hasActiveFilters && <div className="mt-3 flex flex-wrap items-center gap-2"><span className="text-xs font-medium text-muted-foreground"><Filter className="mr-1 inline h-3 w-3" />Filtres actifs</span><button type="button" onClick={clearAllFilters} className="inline-flex min-h-9 items-center gap-1 rounded-md px-2 text-xs text-muted-foreground hover:bg-muted hover:text-destructive"><Trash2 className="h-3 w-3" />Tout effacer</button></div>}</div></section>
      <AnimatePresence>{showFilters && <motion.section initial={{ height: 0, opacity: 1 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden border-b border-border bg-sand/15"><div className="mx-auto max-w-6xl px-4 py-4"><ProjectFilters onFilterChange={setAdvancedFilters} projects={projects ?? []} /></div></motion.section>}</AnimatePresence>
      <section className="px-4 py-8 md:py-10"><div className="mx-auto max-w-6xl">{isError ? <EmptyState icon={FolderSearch} title="Impossible de charger les projets" description="Une erreur temporaire est survenue lors du chargement du catalogue." actionLabel="Réessayer" onAction={() => { void refetch(); }} size="lg" /> : isLoading ? <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">{[1,2,3,4,5,6].map((i) => <ProjectCardSkeleton key={i} />)}</div> : filteredProjects.length ? <><p className="mb-4 flex items-center gap-1.5 text-sm text-muted-foreground"><Building2 className="h-4 w-4 text-forest/60" /><span className="font-semibold text-forest">{filteredProjects.length}</span> projet{filteredProjects.length > 1 ? 's' : ''} trouvé{filteredProjects.length > 1 ? 's' : ''}</p><div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">{filteredProjects.map((project) => <ProjectCard key={project.id} project={project} />)}</div></> : <EmptyState icon={FolderSearch} title="Aucun projet trouvé" description="Essayez d'élargir vos critères de recherche ou de réinitialiser les filtres." actionLabel="Réinitialiser les filtres" onAction={clearAllFilters} size="lg" />}</div></section>
      <ScrollRevealSection icon={LayoutGrid} title="Rechercher un appartement" subtitle="Filtrez par type, budget, surface et plus pour trouver votre futur chez-vous." className="bg-background px-4 py-10" accent><div className="mx-auto max-w-6xl"><div className="mb-6 rounded-xl border border-border bg-card p-5 shadow-sm"><ApartmentSearchFilters filters={apartmentFilters} onFilterChange={setApartmentFilters} districts={districts.length ? districts : undefined} /></div>{hasApartmentFilters && (apartmentsLoading ? <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">{[1,2,3].map((i) => <div key={i} className="h-40 animate-pulse rounded-xl border bg-muted" />)}</div> : apartmentResults?.length ? <div><div className="mb-4 flex items-center justify-between"><p className="text-sm text-muted-foreground"><Home className="mr-1 inline h-4 w-4 text-forest/60" /><span className="font-semibold text-forest">{apartmentResults.length}</span> appartement{apartmentResults.length > 1 ? 's' : ''} trouvé{apartmentResults.length > 1 ? 's' : ''}</p><Button variant="ghost" size="sm" onClick={() => setApartmentFilters(DEFAULT_APARTMENT_FILTERS)}><Trash2 className="mr-1 h-3.5 w-3.5" />Réinitialiser</Button></div><div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">{apartmentResults.map((apt) => <div key={apt.id} className="rounded-xl border border-border bg-card p-4 shadow-sm transition hover:border-forest/30 hover:shadow-md" role="link" tabIndex={0} onClick={() => apt.project && router.goApartment(apt.project.slug, apt.slug)} onKeyDown={(e) => { if ((e.key === 'Enter' || e.key === ' ') && apt.project) { e.preventDefault(); router.goApartment(apt.project.slug, apt.slug); } }}><div className="mb-3 flex items-center justify-between gap-2"><span className="rounded-md bg-forest px-2.5 py-0.5 text-xs font-bold text-white">{apt.typeName}</span><StatusBadge status={apt.status} type="apartment" /></div><div className="mb-3 border-y border-border py-2 text-center"><span className="text-3xl font-bold text-forest">{apt.surface}</span><span className="ml-1 text-sm text-forest/60">m²</span></div><div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground"><span>{apt.bedrooms} chambres</span><span>{apt.bathrooms ?? 0} salles de bain</span><span>{apt.hasParking ? 'Parking disponible' : 'Sans parking'}</span><span>{apt.floor != null ? `Étage ${apt.floor}` : 'Étage —'}</span></div></div>)}</div></div> : null)}</div></ScrollRevealSection>
    </main>
  );
}
