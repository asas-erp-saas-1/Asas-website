...
function parseApartmentTypes(value: string): string[] {
  ...
  try {
    ...
    // Legacy comma-separated storage remains supported at the contract boundary.
  }
...
export function projectCardMatchesFilters(project: PublicProjectCard, filters: ProjectFilters): boolean {
  ...
  const projectPrice = project.priceOnRequest ? 0 : (project.startingPrice ?? 0);
...
export function ProjectFiltersPanel({
  ...
  const { priceMin, priceMax, surfaceMin, surfaceMax } = useMemo(() => {
    ...
    for (const project of projects) {
      if (project.startingPrice && !project.priceOnRequest) {
        pMin = Math.min(pMin, project.startingPrice);
        pMax = Math.max(pMax, project.startingPrice);
      }
