...
interface ProjectFiltersProps { onFilterChange: (filters: ProjectFilters) => void; projects: PublicProjectCard[]; }
...
function countActiveFilters(filters: ProjectFilters, defaults: ProjectFilters) {
  let count = 0;
...
function parseApartmentTypes(value: string): string[] {
  if (!value) return [];
  try { const parsed: unknown = JSON.parse(value); if (Array.isArray(parsed)) return parsed.filter((v): v is string => typeof v === 'string'); } catch { /* legacy comma-separated value */ }
  return value.split(',').map((v) => v.trim()).filter(Boolean);
}

export function projectCardMatchesFilters(project: PublicProjectCard, filters: ProjectFilters): boolean {
...
export function ProjectFiltersPanel({ filters, defaults, onFilterChange, projects }: { filters: ProjectFilters; defaults: ProjectFilters; onFilterChange: (filters: ProjectFilters) => void; projects: PublicProjectCard[] }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
      <div><p className="mb-1.5 text-xs font-medium uppercase tracking-wide text-muted-foreground">Type de projet</p><Select value={filters.projectType ?? '__all__'} onValueChange={(v) => update({ projectType: v === '__all__' ? null : v })}><SelectTrigger className="w-full"><SelectValue placeholder="Tous" /></SelectTrigger><SelectContent><SelectItem value="__all__">Tous</SelectItem>{Object.entries(PROJECT_TYPES).map(([key, label]) => <SelectItem key={key} value={key}>{label}</SelectItem>)}</SelectContent></Select></div>
      <div><p className="mb-1.5 text-xs font-medium uppercase tracking-wide text-muted-foreground">Type d&apos;appartement</p><Select value={filters.apartmentType ?? '__all__'} onValueChange={(v) => update({ apartmentType: v === '__all__' ? null : v })}><SelectTrigger className="w-full"><SelectValue placeholder="Tous" /></SelectTrigger><SelectContent><SelectItem value="__all__">Tous</SelectItem>{Object.entries(APARTMENT_TYPES).map(([key, label]) => <SelectItem key={key} value={key}>{label}</SelectItem>)}</SelectContent></Select></div>
      <div><p className="mb-1.5 text-xs font-medium uppercase tracking-wide text-muted-foreground">Statut</p><Select value={filters.status ?? '__all__'} onValueChange={(v) => update({ status: v === '__all__' ? null : v })}><SelectTrigger className="w-full"><SelectValue placeholder="Tous" /></SelectTrigger><SelectContent><SelectItem value="__all__">Tous</SelectItem>{Object.entries(STATUSES).map(([key, label]) => <SelectItem key={key} value={key}>{label}</SelectItem>)}</SelectContent></Select></div>
    </div>
