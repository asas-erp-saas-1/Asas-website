...
export interface PublicBrochureDownloadProps {
  apartment: PublicApartmentDetail;
  project?: BrochureProject | null;
  variant?: 'full' | 'icon';
}

export function PublicBrochureDownload({ apartment, project, variant = 'full' }: PublicBrochureDownloadProps) {
  ...
  const handleDownload = () => {
    ...
    try {
      ...
      console.error('[PublicBrochureDownload]', error);
  ...
  return (
    <Button
      ...
      className={variant === 'full' ? 'border-white text-white hover:bg-white/10 bg-white/5' : 'text-foreground hover:text-forest hover:bg-forest/5'}
...
function buildBrochureHtml(apartment: PublicApartmentDetail, project?: BrochureProject | null): string {
  ...
  try {
    ...
    if (Array.isArray(parsed)) rooms = parsed.filter(item => item && typeof item.name === 'string' && typeof item.surface === 'number');
  } catch {}

  const spec = (label: string, value: unknown) => `<div class="spec"><span>${esc(label)}</span><strong>${esc(value ?? '—')}</strong></div>`;

  return `<!doctype html><html lang="fr"><head><meta charset="utf-8"><title>Fiche ${esc(apartment.typeName)} - ${esc(projectName)}</title><style>
...
<header class="header"><div><div class="logo">ASAS</div><div class="sub">Agence de Commercialisation Immobilière · Alger</div></div><div class="meta"><strong>FICHE APPARTEMENT</strong><br>Généré le ${esc(date)}<br>Réf. ${esc(apartment.unitNumber ?? apartment.slug)}</div></header>
<section class="hero"><div class="overlay"><div class="type">${esc(apartment.apartmentType)} · ${esc(formatSurface(apartment.surface))}</div><h1 class="title">${esc(apartment.typeName)}</h1><div>${esc(projectName)}${location ? ` · ${esc(location)}` : ''}${apartment.unitNumber ? ` · Unité ${esc(apartment.unitNumber)}` : ''}${apartment.floor != null ? ` · Étage ${esc(apartment.floor)}` : ''}</div></div></section>
<section class="price"><div>PRIX</div><strong>${esc(price)}</strong>${pricePerM2 ? `<div>${esc(pricePerM2)}</div>` : ''}<span class="badge">${esc(status)}</span></section>
<section class="grid"><article class="card"><h2>Caractéristiques</h2><div class="specs">${spec('Surface', formatSurface(apartment.surface))}${spec('Chambres', apartment.bedrooms)}${spec('Salles de bain', apartment.bathrooms)}${spec('Balcons', apartment.balconies)}${spec('Étage', apartment.floor)}${spec('Orientation', apartment.orientation)}${spec('Parking', apartment.hasParking ? `${apartment.parkingSpots ?? 1} place(s)` : 'Non')}${spec('Terrasse', apartment.hasTerrace ? (apartment.terraceSurface ? `${apartment.terraceSurface} m²` : 'Oui') : 'Non')}${spec('Jardin', apartment.hasGarden ? (apartment.gardenSurface ? `${apartment.gardenSurface} m²` : 'Oui') : 'Non')}${spec('Paiement', apartment.paymentPlan)}</div></article><article class="card"><h2>Projet</h2><div class="specs">${spec('Projet', projectName)}${spec('Ville', project?.city ?? apartment.project?.city)}${spec('Quartier', project?.district ?? apartment.project?.district)}${spec('Ascenseur', apartment.project?.hasElevator ? 'Oui' : 'Non')}${spec('Sécurité', apartment.project?.hasSecurity ? 'Oui' : 'Non')}</div></article></section>
${rooms.length ? `<section class="card" style="margin-top:20px"><h2>Distribution</h2><table class="rooms"><thead><tr><th>Pièce</th><th>Surface</th></tr></thead><tbody>${rooms.map(room => `<tr><td>${esc(room.name)}</td><td>${esc(room.surface)} m²</td></tr>`).join('')}</tbody></table></section>` : ''}
<section class="cta"><strong>Intéressé par cet appartement ?</strong><br>Contactez ASAS pour les disponibilités, le prix et les conditions de réservation.<br><strong>${esc(ASAS.phone)} · ${esc(ASAS.email)}</strong></section>
<footer class="footer"><span>ASAS · Agence de Commercialisation Immobilière</span><span>${esc(date)}</span></footer></main></body></html>`;
}
