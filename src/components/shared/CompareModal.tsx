...
interface RowDef { key:string; label:string; icon:React.ComponentType<{className?:string}>; render:(a:Apartment)=>React.ReactNode; compareValue?:(a:Apartment)=>number|null; }
...
export function CompareModal({open,onOpenChange}:CompareModalProps){
 ...
 const sortedApartments=useMemo<Apartment[]>(()=>{if(!apartments)return []; return effectiveCompareList.flatMap(id=>{const apartment=apartments.find(a=>a.id===id); return apartment?[apartment]:[];});},[apartments,effectiveCompareList]);
 const bestApartmentId=useMemo(()=>{let bestId:string|null=null;let bestValue=Infinity;for(const a of sortedApartments){if(a.priceOnRequest||!a.price)continue;const v=a.price/a.surface;if(v<bestValue){bestValue=v;bestId=a.id;}}return bestId;},[sortedApartments]);
