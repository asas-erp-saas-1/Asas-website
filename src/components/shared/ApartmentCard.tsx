...
export function ApartmentCard({ apartment, projectSlug }: ApartmentCardProps) {
  ...
  return (
    <div
      className={`group relative flex min-w-0 flex-col overflow-hidden rounded-xl border border-border bg-card shadow-sm transition-all duration-500 hover:shadow-2xl hover:-translate-y-1.5 hover:border-forest/30 cursor-pointer border-l-4 ${statusBorder} card-glow apartment-border-pulse`}
      ...
      <div className="flex min-w-0 flex-col gap-3 p-4">
        <div className="relative flex min-w-0 items-center justify-between gap-2 -m-1 px-3 py-1 rounded-lg overflow-hidden">
          ...
          <div className="relative flex min-w-0 items-center gap-2 flex-wrap">
            <Badge className="bg-forest text-white text-sm font-bold px-3 py-1 shadow-sm shimmer-badge relative overflow-hidden shrink-0"><span className="relative z-10">{apartment.typeName}</span></Badge>
        ...
        <div className="text-center py-3 border-y border-border"><span className="text-4xl font-bold gradient-text-forest tabular-nums">{apartment.surface}</span><span className="text-lg font-medium text-forest/60 ml-1">m²</span></div>
        ...
        <div className="mt-auto pt-3 border-t border-border">
          {apartment.priceOnRequest || !apartment.price ? <p className="text-lg font-bold text-forest break-words">Prix sur demande</p> : <div><p className="text-2xl font-bold text-forest tabular-nums break-words">{new Intl.NumberFormat('fr-DZ', { maximumFractionDigits: 0 }).format(apartment.price)}<span className="text-base font-semibold ml-1 text-forest/70">DA</span></p>{pricePerSqm != null && <p className="text-xs text-muted-foreground mt-0.5 tabular-nums inline-flex items-center gap-1"><span className="inline-block size-1 rounded-full bg-forest/40 shrink-0" />{new Intl.NumberFormat('fr-DZ', { maximumFractionDigits: 0 }).format(pricePerSqm)} DA/m²</p>}</div>}
        </div>

        <div className="flex items-stretch gap-2 min-w-0">
          <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="flex-1 min-w-0">
            <Button variant="default" size="sm" className="w-full min-h-11 bg-forest hover:bg-forest-dark text-white group-hover:bg-forest-dark group-hover:shadow-md" onClick={(e) => { e.stopPropagation(); router.goApartment(projectSlug, apartment.slug); }}>
