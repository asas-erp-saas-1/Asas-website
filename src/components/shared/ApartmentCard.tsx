...
export function ApartmentCard({ apartment, projectSlug }: ApartmentCardProps) {
  ...
  const statusBorder = apartment.status === 'AVAILABLE'
    ? 'border-l-forest'
    : apartment.status === 'RESERVED' ? 'border-l-gold' : 'border-l-charcoal';

  const pricePerSqm = apartment.price && apartment.surface > 0
    ? Math.round(apartment.price / apartment.surface) : null;
      ...
      <div className="flex flex-col gap-3 p-4">
        <div className="relative flex items-center justify-between gap-2 -m-1 px-3 py-1 rounded-lg overflow-hidden">
          ...
          <div className="relative flex items-center gap-2 flex-wrap">
            <Badge className="bg-forest text-white text-sm font-bold px-3 py-1 shadow-sm shimmer-badge relative overflow-hidden"><span className="relative z-10">{apartment.typeName}</span></Badge>
          ...
          <div className="relative flex items-center gap-2">
            ...
            {apartment.building?.code && <span className="inline-flex items-center rounded-md bg-forest/10 px-2 py-0.5 text-xs font-medium text-forest">{apartment.building.code}</span>}
