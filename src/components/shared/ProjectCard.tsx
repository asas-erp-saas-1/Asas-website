...
function apartmentTypeList(apartmentTypes: string): string[] {
  ...
  try {
    ...
    // Legacy comma-separated values remain supported by the public contract.
  }
...
export function ProjectCard({ project }: ProjectCardProps) {
  ...
  const surfaceRange = project.minSurface && project.maxSurface
    ? `${project.minSurface} - ${project.maxSurface} m²`
    : project.minSurface
      ? `À partir de ${project.minSurface} m²`
      : null;

  const deliveryInfo = project.deliveryYear && project.deliveryQuarter
    ? `${project.deliveryQuarter} ${project.deliveryYear}`
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-sm transition-[transform,box-shadow,border-color] duration-300 hover:-translate-y-0.5 hover:border-forest/30 hover:shadow-xl">
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        <button type="button" onClick={handleToggleFavorite} aria-label={isFav ? 'Retirer des favoris' : 'Ajouter aux favoris'} aria-pressed={isFav} className="absolute left-3 top-3 z-20 inline-flex size-9 items-center justify-center rounded-full border border-white/30 bg-black/20 text-white backdrop-blur-md transition hover:scale-105 hover:bg-black/35 focus-visible:ring-2 focus-visible:ring-white">
