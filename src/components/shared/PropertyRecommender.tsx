...
const cardVariants = { hidden: { opacity: 0, x: 30 }, visible: { opacity: 1, x: 0, transition: { type: 'spring' as const, stiffness: 260, damping: 24 } } };
...
function PropertyRecommenderSkeleton() {
  return <section className="py-8 sm:py-10"><div className="mb-6 flex items-center gap-3"><span className="flex size-9 items-center justify-center rounded-lg bg-forest/10"><Sparkles className="size-5 text-forest" /></span><h3 className="text-xl font-bold tracking-tight text-foreground sm:text-2xl">Appartements similaires</h3></div><div className="flex gap-4 overflow-x-auto md:grid md:grid-cols-3 md:overflow-visible">{Array.from({ length: 3 }).map((_, i) => <div key={`skeleton-${i}`} className="min-w-[280px] overflow-hidden rounded-xl border border-border bg-card md:min-w-0 animate-pulse"><div className="space-y-3 p-4"><div className="h-6 w-1/3 rounded bg-muted" /><div className="mx-auto h-8 w-1/2 rounded bg-muted" /><div className="h-4 w-2/3 rounded bg-muted" /><div className="h-5 w-1/3 rounded bg-muted" /></div></div>)}</div></section>;
}
