...
export function AdminLeadsPremiumWorkspace() {
  const { canMutate, loading: roleLoading } = useAdminRole();
  ...
  async function addNote() {
    if (!canMutate) {
      setNotesError('Vous n’avez pas les privilèges nécessaires pour ajouter une note.');
      return;
    }
  ...
  async function updateStatus() {
    if (!canMutate) {
      setFeedback({ type: 'error', text: 'Vous n’avez pas les privilèges nécessaires pour modifier le statut d’un lead.' });
  ...
  function openStatusForLead(lead: Lead) {
    if (!canMutate) {
      setFeedback({ type: 'error', text: 'Votre rôle est en lecture seule pour le cycle de vie des leads.' });
      return;
    }
  ...
  function openProject(projectId: string) {
    navigateAdminRoute({ workspace: 'projects', entity: 'project', entityId: projectId });
  }

  function openApartment(apartmentId: string) {
    navigateAdminRoute({ workspace: 'apartments', entity: 'apartment', entityId: apartmentId });
  }
    ...
    <section className="admin-leads-workspace w-full" aria-labelledby="leads-workspace-title">
      <div className="mx-auto max-w-[1500px] space-y-5">
        ...
        : <Card className="overflow-hidden"><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Lead</TableHead><TableHead>Contact</TableHead><TableHead>Intention</TableHead><TableHead>Projet / lot</TableHead><TableHead>Source</TableHead><TableHead>Statut</TableHead><TableHead>Créé</TableHead><TableHead className="text-right">Action</TableHead></TableRow></TableHeader><TableBody>{leads.map((lead) => {
          const allowedStatuses = getAllowedLeadStatusTransitions(lead.status);
          return <TableRow key={lead.id} className={loading ? 'opacity-70' : undefined}>
          ...
          <TableCell><div className="min-w-[180px] space-y-1">{lead.projectName ? <button type="button" className="block text-left font-medium hover:underline" onClick={() => lead.projectId && openProject(lead.projectId)} disabled={!lead.projectId}>{lead.projectName}</button> : null}{lead.apartmentName ? <button type="button" className="block text-left text-xs text-muted-foreground hover:underline" onClick={() => lead.apartmentId && openApartment(lead.apartmentId)} disabled={!lead.apartmentId}>{lead.apartmentName}</button> : !lead.projectName ? <span>—</span> : null}</div></TableCell>
          <TableCell className="text-sm">{lead.source ?? '—'}</TableCell>
          <TableCell><select aria-label={`Statut de ${lead.name}`} value={lead.status} onChange={(event) => { if (event.target.value !== lead.status) setStatusRequest({ lead, nextStatus: event.target.value }); }} disabled={roleLoading || mutationBusy || !canMutate} title={!canMutate ? 'Privilèges insuffisants' : 'Modifier le statut'} className="h-9 rounded-md border border-input bg-background px-2 text-sm">{allowedStatuses.map((value) => <option key={value} value={value}>{labelOf(STATUS_OPTIONS, value)}</option>)}</select></TableCell>
        ...
        })}</TableBody></Table></div><div className="flex flex-col gap-3 border-t p-3 sm:flex-row sm:items-center sm:justify-between"><span className="text-xs text-muted-foreground">Page {meta.page} sur {meta.totalPages}</span><nav aria-label="Pagination des leads" className="flex items-center gap-1"><Button variant="outline" size="sm" onClick={() => { const next = Math.max(1, page - 1); setPage(next); syncRoute({ page: next }); }} disabled={loading || page <= 1} aria-label="Page précédente" className="gap-1"><ChevronLeft className="h-4 w-4" /> Précédente</Button><Button variant="outline" size="sm" onClick={() => { const next = Math.min(meta.totalPages, page + 1); setPage(next); syncRoute({ page: next }); }} disabled={loading || page >= meta.totalPages} aria-label="Page suivante" className="gap-1">Suivante <ChevronRight className="h-4 w-4" /> </Button></nav></div></Card>}
      </div>

      <Dialog open={selectedLead !== null} onOpenChange={(open) => { if (!open) setSelectedLead(null); }}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-xl"><DialogHeader><DialogTitle>Détails du lead{selectedLead ? ` — ${selectedLead.name}` : ''}</DialogTitle><DialogDescription>Contexte commercial et coordonnées disponibles pour traiter le prospect.</DialogDescription></DialogHeader>{selectedLead && <div className="grid gap-4 text-sm"><div className="flex flex-wrap gap-2"><Badge variant="secondary">{labelOf(STATUS_OPTIONS, selectedLead.status)}</Badge><Badge variant="outline">{labelOf(INTENT_OPTIONS, selectedLead.intent)}</Badge></div><div className="grid gap-3 sm:grid-cols-2"><div><p className="text-xs font-medium text-muted-foreground">Téléphone</p><a className="inline-flex min-h-11 items-center gap-2 hover:underline" href={`tel:${selectedLead.phone}`}><Phone className="h-4 w-4" />{selectedLead.phone}</a></div><div><p className="text-xs font-medium text-muted-foreground">Email</p>{selectedLead.email ? <a className="inline-flex min-h-11 items-center gap-2 break-all hover:underline" href={`mailto:${selectedLead.email}`}><Mail className="h-4 w-4" />{selectedLead.email}</a> : <p className="mt-2">—</p>}</div><div><p className="text-xs font-medium text-muted-foreground">Projet</p>{selectedLead.projectName && selectedLead.projectId ? <button type="button" className="mt-1 text-left font-medium hover:underline" onClick={() => openProject(selectedLead.projectId!)}>{selectedLead.projectName}</button> : <p className="mt-1">{selectedLead.projectName ?? '—'}</p>}</div><div><p className="text-xs font-medium text-muted-foreground">Lot / appartement</p>{selectedLead.apartmentName && selectedLead.apartmentId ? <button type="button" className="mt-1 text-left font-medium hover:underline" onClick={() => openApartment(selectedLead.apartmentId!)}>{selectedLead.apartmentName}</button> : <p className="mt-1">{selectedLead.apartmentName ?? '—'}</p>}</div><div><p className="text-xs font-medium text-muted-foreground">Source</p><p className="mt-1">{selectedLead.source ?? '—'}</p></div><div><p className="text-xs font-medium text-muted-foreground">Assigné à</p><p className="mt-1">{selectedLead.assignedTo ?? 'Non assigné'}</p></div><div><p className="text-xs font-medium text-muted-foreground">Prochain suivi</p><p className="mt-1">{formatDate(selectedLead.followUpDate)}</p></div><div><p className="text-xs font-medium text-muted-foreground">Créé le</p><p className="mt-1">{formatDateTime(selectedLead.createdAt)}</p></div></div><div className="rounded-md border bg-muted/20 p-3"><p className="mb-1 text-xs font-medium text-muted-foreground">Message</p><p className="whitespace-pre-wrap leading-6">{selectedLead.message?.trim() || 'Aucun message fourni.'}</p></div></div>}<DialogFooter>{selectedLead && <Button variant="outline" onClick={() => openStatusForLead(selectedLead)} disabled={roleLoading || !canMutate} title={!canMutate ? 'Privilèges insuffisants' : 'Modifier le statut'}>Changer le statut</Button>}<Button variant="outline" onClick={() => setSelectedLead(null)}>Fermer</Button></DialogFooter></DialogContent></Dialog>

      <Dialog open={notesLead !== null} onOpenChange={(open) => { if (!open && !noteSaving) setNotesLead(null); }}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-xl"><DialogHeader><DialogTitle>Notes — {notesLead?.name ?? ''}</DialogTitle><DialogDescription>Notes internes de suivi commercial. Elles ne sont jamais publiées sur le site.</DialogDescription></DialogHeader><div className="space-y-4"><div className="space-y-2"><label htmlFor="lead-note" className="text-sm font-medium">Ajouter une note</label><Textarea id="lead-note" value={noteDraft} onChange={(event) => setNoteDraft(event.target.value)} placeholder="Compte rendu d’appel, prochaine action, objection, rendez-vous…" rows={4} disabled={roleLoading || noteSaving || !canMutate} aria-disabled={!canMutate} /><div className="flex justify-end"><Button onClick={addNote} disabled={roleLoading || noteSaving || !canMutate || !noteDraft.trim()} className="gap-2">{noteSaving && <Loader2 className="h-4 w-4 animate-spin" />} Ajouter la note</Button></div></div>{notesError && <div role="alert" className="rounded-md border border-red-200 bg-red-50 p-3 text-sm text-red-800">{notesError}</div>}<div className="space-y-3"><div className="flex items-center justify-between"><h3 className="text-sm font-semibold">Historique</h3>{notesLoading && <Loader2 className="h-4 w-4 animate-spin" aria-label="Chargement" />}</div>{!notesLoading && notes.length === 0 ? <p className="rounded-md border border-dashed p-6 text-center text-sm text-muted-foreground">Aucune note pour le moment.</p> : <div className="space-y-3">{notes.map((note) => <article key={note.id} className="rounded-lg border bg-muted/20 p-3"><div className="mb-1 flex flex-wrap items-center justify-between gap-2 text-xs text-muted-foreground"><span>{note.authorEmail ?? 'Inconnu'}</span><time dateTime={note.createdAt}>{formatDateTime(note.createdAt)}</time></div><p className="whitespace-pre-wrap text-sm leading-6">{note.body}</p></article>)}</div>}</div></div><DialogFooter><Button variant="outline" onClick={() => setNotesLead(null)} disabled={noteSaving}>Fermer</Button></DialogFooter></DialogContent></Dialog>

      <Dialog open={statusRequest !== null} onOpenChange={(open) => { if (!open && !mutationBusy) setStatusRequest(null); }}><DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle>Mettre à jour le statut</DialogTitle><DialogDescription>{statusRequest ? `Choisissez le nouveau statut de « ${statusRequest.lead.name} ».` : 'Choisissez un nouveau statut.'}</DialogDescription></DialogHeader>{statusRequest && <label className="space-y-2 text-sm font-medium"><span>Nouveau statut</span><select autoFocus value={statusRequest.nextStatus} onChange={(event) => setStatusRequest((current) => current ? { ...current, nextStatus: event.target.value } : current)} disabled={roleLoading || mutationBusy || !canMutate} className="flex h-11 w-full rounded-md border border-input bg-background px-3 py-2 text-sm">{getAllowedLeadStatusTransitions(statusRequest.lead.status).map((value) => <option key={value} value={value}>{labelOf(STATUS_OPTIONS, value)}</option>)}</select></label>}<DialogFooter><Button variant="outline" onClick={() => setStatusRequest(null)} disabled={mutationBusy}>Annuler</Button><Button onClick={updateStatus} disabled={roleLoading || mutationBusy || !canMutate || !statusRequest || statusRequest.nextStatus === statusRequest.lead.status} className="gap-2">{mutationBusy && <Loader2 className="h-4 w-4 animate-spin" />} Enregistrer</Button></DialogFooter></DialogContent></Dialog>
    </section>
  );
}
