  ...
  return (
    <section className="admin-buildings-workspace w-full" aria-labelledby="buildings-workspace-title">
      <div className="mx-auto max-w-[1500px] space-y-5">
        ...
        : <Card className="overflow-hidden"><div className="overflow-x-auto"><Table className="min-w-[760px]"><TableHeader><TableRow><TableHead>Bâtiment</TableHead><TableHead>Projet</TableHead><TableHead>Code</TableHead><TableHead>Étages</TableHead><TableHead>Ascenseur</TableHead><TableHead>Lots</TableHead><TableHead>Ordre</TableHead></TableRow></TableHeader><TableBody>{buildings.map((building) => <TableRow key={building.id}><TableCell><div className="min-w-[180px]"><div className="mb-1 text-[11px] text-muted-foreground">{(() => { const readiness = operationalReadiness.find((item) => item.id === building.id); return readiness ? `${readiness.completionRatio}% renseigné` : null; })()}</div><div className="font-medium">{building.name}</div>{building.nameAr && <div dir="rtl" className="text-xs text-muted-foreground">{building.nameAr}</div>}<div className="text-xs text-muted-foreground">{building.slug}</div></div></TableCell><TableCell><div className="min-w-[150px] text-sm">{building.project?.name ?? '—'}</div></TableCell><TableCell><Badge variant="secondary">{building.code}</Badge></TableCell><TableCell className="text-sm">{building.floors}</TableCell><TableCell className="text-sm">{building.hasElevator ? 'Oui' : 'Non'}</TableCell><TableCell className="text-sm font-medium">{building.apartmentCount}</TableCell><TableCell className="text-sm">{building.order}</TableCell></TableRow>)}</TableBody></Table></div><div className="flex flex-col gap-3 border-t p-3 sm:flex-row sm:items-center sm:justify-between"><span className="text-xs text-muted-foreground">Page {meta.page} sur {meta.totalPages}</span><nav aria-label="Pagination des bâtiments" className="flex items-center gap-1"><Button variant="outline" size="sm" onClick={() => { const next = Math.max(1, page - 1); setPage(next); syncRoute({ page: next }); }} disabled={loading || page <= 1} aria-label="Page précédente" className="gap-1"><ChevronLeft className="h-4 w-4" /> Précédente</Button><Button variant="outline" size="sm" onClick={() => { const next = Math.min(meta.totalPages, page + 1); setPage(next); syncRoute({ page: next }); }} disabled={loading || page >= meta.totalPages} aria-label="Page suivante" className="gap-1">Suivante <ChevronRight className="h-4 w-4" /></Button></nav></div></Card>}
      </div>
