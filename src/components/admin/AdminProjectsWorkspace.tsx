...
export function AdminProjectsWorkspace() {
  ...
  useEffect(() => { const timer = window.setTimeout(() => { const normalized = search.trim(); if (normalized !== (getAdminRoute().search ?? '')) navigateAdminRoute({ workspace: 'projects', search: normalized || undefined, filters: { status }, page: 1 }, 'replace'); setDebouncedSearch(normalized); }, 300); return () => window.clearTimeout(timer); }, [search, status]);
