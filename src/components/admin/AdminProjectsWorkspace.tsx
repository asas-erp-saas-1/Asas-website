...
import { Archive, Building2, ChevronLeft, Eye, EyeOff, Filter, Loader2, RefreshCw, Search, X } from 'lucide-react';
...
export function AdminProjectsWorkspace() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    getJson<{ data?: Project[] }>('/api/admin/projects', { signal: controller.signal })
  ...
  const visibleProjects = projects.filter((project) => {
    ...
    const haystack = `${project.name} ${project.slug} ${project.city} ${project.district} ${project.developer?.name ?? ''}`.toLocaleLowerCase('fr-FR');
      ...
      <div className="mx-auto max-w-[1400px] space-y-5">
        <header className="flex flex-col gap-4 border-b border-border pb-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 id="projects-workspace-title" className="text-2xl font-bold text-charcoal sm:text-3xl">Projets</h1>
            <p className="mt-1 text-sm text-muted-foreground">Vue opérationnelle du portefeuille immobilier avec recherche et actions sécurisées.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="outline" size="sm" onClick={refresh} disabled={loading || refreshing} className="gap-2" aria-label="Actualiser les projets"><RefreshCw className={`h-4 w-4 ${refreshing ? 'animate-spin' : ''}`} /> Actualiser</Button>
          </div>
        </header>
        ...
        <Card>
          <CardHeader className="pb-3"><div className="flex items-center justify-between gap-3"><CardTitle className="flex items-center gap-2 text-base"><Filter className="h-4 w-4" /> Filtres</CardTitle>{hasFilters && <Button variant="ghost" size="sm" onClick={clearFilters} className="gap-2"><X className="h-4 w-4" /> Effacer</Button>}</div></CardHeader>
          <CardContent><div className="grid gap-3 sm:grid-cols-2">
            <label className="space-y-1.5 text-sm font-medium"><span>Recherche</span><div className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Projet, ville, quartier, promoteur…" className="pl-9" /></div></label>
            <label className="space-y-1.5 text-sm font-medium"><span>Statut</span><select value={status} onChange={(event) => setStatus(event.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"><option value="all">Tous les statuts</option>{STATUS_OPTIONS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
          </div></CardContent>
        </Card>

        <div aria-live="polite" className="flex flex-wrap items-center justify-between gap-2 text-sm text-muted-foreground"><span>{visibleProjects.length.toLocaleString('fr-FR')} projet{visibleProjects.length > 1 ? 's' : ''} affiché{visibleProjects.length > 1 ? 's' : ''}</span>{loading && <span className="inline-flex items-center gap-2"><Loader2 className="h-4 w-4 animate-spin" /> Chargement…</span>}</div>
