...
const STATUS_OPTIONS = [['AVAILABLE', 'Disponible'], ['COMING_SOON', 'Bientôt'], ['SOLD_OUT', 'Épuisé'], ['DRAFT', 'Brouillon']] as const;
...
export function AdminProjectsWorkspace() {
  ...
  const normalizedSearch = debouncedSearch; const hasFilters = status !== 'all' || normalizedSearch !== '';
  function refresh() { setRefreshing(true); setLoading(true); setRetryKey((value) => value + 1); }
  function syncRoute(next: { search?: string; status?: string; page?: number }) { navigateAdminRoute({ workspace: 'projects', search: next.search ?? search, filters: { status: next.status ?? status }, page: next.page ?? page }, 'replace'); }
  function clearFilters() { setSearch(''); setStatus('all'); setPage(1); setLoading(true); syncRoute({ search: '', status: 'all', page: 1 }); }
  useEffect(() => { const timer = window.setTimeout(() => { const normalized = search.trim(); if (normalized !== (getAdminRoute().search ?? '')) navigateAdminRoute({ workspace: 'projects', search: normalized || undefined, filters: { status }, page: 1 }, 'replace'); setDebouncedSearch(normalized); setLoading(true); }, 300); return () => window.clearTimeout(timer); }, [search, status]);
  function changeSearch(value: string) { setSearch(value); setPage(1); setLoading(true); }
  function changeStatus(value: string) { setStatus(value); setPage(1); setLoading(true); syncRoute({ status: value, page: 1 }); }

  async function executeMutation() {
    if (!pendingAction || !canMutate) return;
    const { kind, project } = pendingAction; setMutationError(null); setMutationSuccess(null);
    try {
      if (kind === 'publish') { await getJson(`/api/admin/projects/${encodeURIComponent(project.slug)}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ published: !project.published }) }); setMutationSuccess(!project.published ? `« ${project.name} » est maintenant publié.` : `« ${project.name} » a été retiré de la publication.`); }
      else { await getJson(`/api/admin/projects/${encodeURIComponent(project.slug)}`, { method: 'DELETE' }); setMutationSuccess(`« ${project.name} » a été archivé.`); }
  ...
  return (
    <section className="admin-projects-workspace w-full" aria-labelledby="projects-workspace-title">
      <div className="mx-auto max-w-[1400px] space-y-5">
        <header className="flex flex-col gap-4 border-b border-border pb-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="mb-1 text-xs font-semibold uppercase tracking-[0.16em] text-forest">Catalogue</p><h1 id="projects-workspace-title" className="text-2xl font-bold text-charcoal sm:text-3xl">Projets</h1><p className="mt-1 text-sm text-muted-foreground">Vue opérationnelle du portefeuille immobilier avec recherche, pagination et actions sécurisées.</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" size="sm" onClick={() => navigateAdminRoute({ workspace: 'dashboard' })} className="gap-2"><ChevronLeft className="h-4 w-4" /> Retour au tableau de bord</Button><Button variant="outline" size="sm" onClick={refresh} disabled={loading || refreshing} className="gap-2" aria-label="Actualiser les projets"><RefreshCw className={`h-4 w-4 ${refreshing ? 'animate-spin' : ''}`} /> Actualiser</Button></div></header>
        <div className="grid gap-3 sm:grid-cols-3" aria-label="État opérationnel de la page"><Card><CardContent className="p-4"><p className="text-xs font-medium text-muted-foreground">Projets visibles</p><p className="mt-1 text-2xl font-semibold">{operationalReadiness.length}</p></CardContent></Card><Card><CardContent className="p-4"><p className="text-xs font-medium text-muted-foreground">Informations incomplètes détectées</p><p className="mt-1 text-2xl font-semibold">{operationalReadiness.filter((item) => item.state === 'incomplete').length}</p></CardContent></Card><Card><CardContent className="p-4"><p className="text-xs font-medium text-muted-foreground">Prêts selon les données disponibles</p><p className="mt-1 text-2xl font-semibold">{operationalReadiness.filter((item) => item.state === 'completed' || item.state === 'ready').length}</p></CardContent></Card></div>
