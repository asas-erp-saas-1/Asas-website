        ...
        <div className="mx-auto max-w-[1400px] space-y-5">
          ...
          {detailLoading ? <Card><CardContent className="flex items-center justify-center gap-2 py-16 text-sm text-muted-foreground"><Loader2 className="h-5 w-5 animate-spin" /> Chargement…</CardContent></Card> : detailError ? <Card role="alert"><CardContent className="space-y-3 py-12 text-center"><p className="font-semibold">Impossible de charger l’appartement</p><p className="text-sm text-muted-foreground">{detailError}</p><Button onClick={() => setDetailRetryKey((key) => key + 1)}>Réessayer</Button></CardContent></Card> : detail ? <>
            ...
            <div className="grid gap-5 lg:grid-cols-2">
              ...
              <Card><CardHeader><CardTitle className="text-base">Commercial & publication</CardTitle></CardHeader><CardContent className="space-y-3 text-sm"><div className="flex flex-wrap items-center gap-2"><Button size="sm" disabled={detailMutationBusy || role === 'VIEWER'} onClick={() => updateApartmentDetail({ published: !detail.published })}>{detailMutationBusy ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : detail.published ? <EyeOff className="mr-2 h-4 w-4" /> : <Eye className="mr-2 h-4 w-4" />}{detail.published ? 'Dépublier' : 'Publier'}</Button>{detailMutationError && <><span role="alert" className="text-sm text-destructive">{detailMutationError}</span><Button variant="outline" size="sm" onClick={() => updateApartmentDetail({ published: !detail.published })}>Réessayer</Button></>}</div><div><span className="text-muted-foreground">Prix de vente (DZD)</span><div className="mt-1 flex gap-2"><Input inputMode="decimal" type="number" min="0" step="1000" value={priceDraft || (detail.price != null ? String(detail.price) : '')} onChange={(e) => setPriceDraft(e.target.value)} aria-label="Prix en DZD" /><Button size="sm" disabled={detailMutationBusy || role === 'VIEWER' || !priceDraft} onClick={() => updateApartmentDetail({ price: Number(priceDraft), priceOnRequest: false })}>Enregistrer</Button></div></div><div><span className="text-muted-foreground">Plan de paiement</span><p className="font-medium whitespace-pre-wrap">{detail.paymentPlan ?? 'Non renseigné'}</p></div><div className="flex flex-wrap gap-2"><div className="flex items-center gap-2"><span className="text-xs text-muted-foreground">Statut</span><Select value={statusDraft || detail.status} onValueChange={(value) => setStatusDraft(value)} disabled={detailMutationBusy || role === 'VIEWER'}><SelectTrigger className="w-[190px]" aria-label="Statut appartement"><SelectValue /></SelectTrigger><SelectContent>{['AVAILABLE','RESERVED','SOLD','COMING_SOON','OFF_MARKET','DRAFT'].map((value) => <SelectItem key={value} value={value}>{value}</SelectItem>)}</SelectContent></Select><Button size="sm" variant="outline" disabled={detailMutationBusy || role === 'VIEWER' || !statusDraft || statusDraft === detail.status} onClick={() => updateApartmentDetail({ status: statusDraft })}>Appliquer</Button></div>{statusAnchor}{detail.published ? 'Publié' : 'Brouillon'}</Badge><Badge variant="outline">{detail.archived ? 'Archivé' : 'Actif'}</Badge></div><div><span className="text-muted-foreground">SEO</span><p className="font-medium">{detail.seoTitle ?? 'Non renseigné'}</p><p className="text-xs text-muted-foreground">{detail.canonicalUrl ?? 'Canonical non renseignée'}</p></div></CardContent></Card>
            </div>
