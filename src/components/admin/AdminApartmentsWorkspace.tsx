...
export function AdminApartmentsWorkspace() {
  const { canMutate, canAdminister, loading: roleLoading } = useAdminRole();
  ...
  async function executeMutation() {
    ...
    if (kind === 'archive' && !canAdminister) {
      setMutationError('Vous n’avez pas les privilèges nécessaires pour archiver un appartement.');
      setPendingAction(null);
      return;
    }
    if (kind === 'publish' && !canMutate) {
      setMutationError('Vous n’avez pas les privilèges nécessaires pour modifier la publication.');
      setPendingAction(null);
      return;
    }
  ...
  async function updateApartmentDetail(patch: Record<string, unknown>) {
    ...
    if (!canMutate) {
      setDetailMutation({ state: 'recoverable-error', error: 'Vous n’avez pas les privilèges nécessaires pour modifier cet appartement.' });
      return;
    }
          ...
          {detailLoading ? <Card><CardContent className="flex items-center justify-center gap-2 py-16 text-sm text-muted-foreground"><Loader2 className="h-5 w-5 animate-spin" /> Chargement…</CardContent></Card> : detailError ? <Card role="alert"><CardContent className="space-y-3 py-12 text-center"><p className="font-semibold">Impossible de charger l’appartement</p><p className="text-sm text-muted-foreground">{detailError}</p><Button onClick={() => setDetailRetryKey((key) => key + 1)}>Réessayer</Button></CardContent></Card> : detail ? <>
            ...
            <div className="grid gap-5 lg:grid-cols-2">
              ...
              <Card><CardHeader><CardTitle className="text-base">Commercial & publication</CardTitle></CardHeader><CardContent className="space-y-4 text-sm">
                <div className="flex flex-wrap items-end gap-2"><div className="min-w-[180px] flex-1"><span className="text-muted-foreground">Prix de vente (DZD)</span><Input className="mt-1" inputMode="decimal" type="number" min="0" step="1000" value={priceDraft || (detail.price != null ? String(detail.price) : '')} onChange={(e) => setPriceDraft(e.target.value)} aria-label="Prix en DZD" /></div><Button size="sm" disabled={roleLoading || detailMutationBusy || !canMutate || !priceDraft} onClick={() => updateApartmentDetail({ price: Number(priceDraft), priceOnRequest: false })}>Enregistrer</Button></div>
                <div className="flex flex-wrap items-center gap-2"><span className="text-muted-foreground">Statut</span><Select value={statusDraft || detail.status} onValueChange={setStatusDraft} disabled={roleLoading || detailMutationBusy || !canMutate}><SelectTrigger className="w-[190px]" aria-label="Statut appartement"><SelectValue /></SelectTrigger><SelectContent>{[detail.status, ...(Object.keys({ AVAILABLE: true, SOLD: true, COMING_SOON: true, OFF_MARKET: true, DRAFT: true } as const)).filter((value) => value !== detail.status && canTransitionApartmentStatus(detail.status, value))].map((value) => <SelectItem key={value} value={value}>{value}</SelectItem>)}</SelectContent></Select><Button size="sm" variant="outline" disabled={roleLoading || detailMutationBusy || !canMutate || !statusDraft || statusDraft === detail.status} onClick={() => updateApartmentDetail({ status: statusDraft })}>Appliquer</Button></div>
                <div className="flex flex-wrap items-center gap-2"><Button size="sm" disabled={roleLoading || detailMutationBusy || !canMutate} onClick={() => {
                  if (!canMutate) return;
                  if (!detail.published) {
