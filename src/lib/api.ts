...
export function useApartmentsByIds(ids: string[]) {
  return useQuery({
    ...
    select: (projects: Project[]) => projects.flatMap(project => (project.apartments ?? []).map(apartment => ({ ...apartment, project }))).filter(a => new Set(ids).has(a.id)),
...
export function useApartmentSearch(params: Record<string, string | number | undefined>) {
  ...
  return useQuery({ queryKey: ['apartments', 'search', qs], queryFn: () => getJson<Apartment[]>(`${API_BASE}/apartments${qs ? `?${qs}` : ''}`), enabled: Object.values(params).some(v => v !== undefined && v !== '') });
}
