...
export function toPublicApartmentDetail(apartment: Record<string, unknown>): PublicApartmentDetail {
  return {
    ...
    apartmentType: String(apartment.apartmentType),
    typeName: String(apartment.typeName),
    typeNameAr: optionalString(apartment.typeNameAr),
    surface: Number(apartment.surface),
    floor: typeof apartment.floor === 'number' ? apartment.floor : undefined,
    ...
    hasParking: Boolean(apartment.hasParking),
    parkingSpots: typeof apartment.parkingSpots === 'number' ? apartment.parkingSpots : undefined,
    hasTerrace: Boolean(apartment.hasTerrace),
    terraceSurface: typeof apartment.terraceSurface === 'number' ? apartment.terraceSurface : undefined,
    hasGarden: Boolean(apartment.hasGarden),
    gardenSurface: typeof apartment.gardenSurface === 'number' ? apartment.gardenSurface : undefined,
    ...
    paymentPlan: optionalString(apartment.paymentPlan),
    paymentPlanAr: optionalString(apartment.paymentPlanAr),
    ...
    building: apartment.building && typeof apartment.building === 'object'
      ? toPublicBuilding(apartment.building as Record<string, unknown>)
      : undefined,
    images: Array.isArray(apartment.images)
      ? apartment.images.map((image) => toPublicApartmentImage(image as Record<string, unknown>))
      : [],
  };
}
...
export function toPublicProjectDetail(project: Record<string, unknown>): PublicProjectDetail {
  return {
    ...
    buildings: Array.isArray(project.buildings)
      ? project.buildings.map((building) => toPublicBuilding(building as Record<string, unknown>))
      : [],
    apartments: Array.isArray(project.apartments)
      ? project.apartments.map((apartment) => toPublicApartmentDetail(apartment as Record<string, unknown>))
      : [],
    amenities: Array.isArray(project.amenities)
      ? project.amenities.map((amenity) => toPublicAmenity(amenity as Record<string, unknown>))
      : [],
    images: Array.isArray(project.images)
      ? project.images.map((image) => toPublicProjectImage(image as Record<string, unknown>))
      : [],
    developer: project.developer && typeof project.developer === 'object'
      ? toPublicDeveloper(project.developer as Record<string, unknown>)
      : undefined,
  };
}
