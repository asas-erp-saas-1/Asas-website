 ...
 * List apartments with filters (projectId, status, type)
 *
 * Query params:
 *   projectId  – filter by project ID
 *   projectSlug – filter by project slug
 *   status     – filter by status
 *   type       – filter by apartmentType
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (publishedStr !== null && publishedStr !== undefined && publishedStr !== '') {
      where.published = publishedStr === 'true';
    }
    ...
    const result = apartments.map((a) => ({
      ...
      status: a.status,
      price: a.price,
      priceOnRequest: a.priceOnRequest,
      published: a.published,
      order: a.order,
      building: a.building,
      project: a.project,
      heroImage: a.images[0]?.url ?? null,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!body.slug || !body.projectId || !body.typeName || !body.surface || body.bedrooms === undefined) {
      return withSecurityHeaders(NextResponse.json(
        { error: 'Missing required fields: slug, projectId, typeName, surface, bedrooms' },
        { status: 400 }
      ));
    }

    // Check slug uniqueness
    const existing = await db.apartment.findUnique({ where: { slug: body.slug } });
    if (existing) {
      return withSecurityHeaders(NextResponse.json(
    ...
    const apartment = await db.apartment.create({
      data: {
        ...
        apartmentType: body.apartmentType ?? 'F3',
        typeName: body.typeName,
        typeNameAr: body.typeNameAr ?? null,
