...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [apartments, total] = await Promise.all([
      db.apartment.findMany({
        ...
        include: {
          ...
          // Keep this projection aligned with the live PostgreSQL schema. The legacy
          // captionAr column is absent in production and must not be selected here.
          imagesRelation: {
            where: { type: 'hero' },
            take: 1,
    ...
    const result = apartments.map((a) => ({
      ...
      hasParking: a.hasParking ?? a.parking,
      ...
      building: a.building,
      project: a.project,
      heroImage: a.imagesRelation[0]?.url ?? null,
