 ...
 * Get single project with full details (buildings, apartments, images, amenities)
 */
 ...
 * Update project (name, description, status, featured, published, etc.)
 */
export async function PUT(
  ...
  try {
    ...
    if (body.status && !validProjectStatuses.includes(body.status)) {
      return withSecurityHeaders(NextResponse.json(
        { error: `Invalid status. Valid values: ${validProjectStatuses.join(', ')}` },
        { status: 400 }
      ));
    }
    ...
    const allowedFields = [
      'name', 'nameAr', 'tagline', 'taglineAr', 'description', 'descriptionAr',
      'city', 'cityAr', 'district', 'districtAr', 'address', 'addressAr',
      'latitude', 'longitude', 'projectType', 'status', 'apartmentTypes',
      'minSurface', 'maxSurface', 'deliveryYear', 'deliveryQuarter',
    ...
    await logAudit({
      request, session,
      action: priceChanged ? 'PRICE_CHANGE' : 'UPDATE_PROJECT',
      entityType: 'Project',
      entityId: project.id,
 ...
 * Archive project (set archived: true, not actual delete)
 */
export async function DELETE(
  ...
  try {
    ...
    await logAudit({
      ...
      entityId: project.id,
      entitySlug: project.slug,
      before: { name: existing.name, slug: existing.slug, published: existing.published, archived: existing.archived },
      after: { name: project.name, slug: project.slug, published: project.published, archived: project.archived },
    });
